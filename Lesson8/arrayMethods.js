...
function map( arr, transform ) {
    ...
    for (let i=0; i<arr.length; i++) {
        // result[i] = transform(arr[i], i, arr);
...
console.log( filter(
    [1, 2, 3, 3, 3, 4, 5, 6],
    function checkHandler(el) { return el < 5 && el > 2; }
)); // [3, 3, 3, 4]
...
function concat(arr1, arr2) {
    ...
    for (let i=0; i < arguments.length; i++) {
        ...
        // for (let j=0; j<addedArr.length; j++) {
        //     result.push(addedArr[j]);
        // }
    }
